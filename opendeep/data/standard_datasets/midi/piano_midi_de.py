...
class PianoMidiDe(FileDataset):
    ...
    def __init__(self, dataset_dir='../../datasets'):
        ...
        train_filenames = os.path.join(self.dataset_location, 'Piano-midi.de', 'train', '*.mid')
        valid_filenames = os.path.join(self.dataset_location, 'Piano-midi.de', 'valid', '*.mid')
        test_filenames = os.path.join(self.dataset_location, 'Piano-midi.de', 'test', '*.mid')
        # glob the files
        ...
        self.train = dataset_shared(numpy.concatenate(train_datasets), name='piano_train', borrow=True)
        self.valid = dataset_shared(numpy.concatenate(valid_datasets), name='piano_valid', borrow=True)
